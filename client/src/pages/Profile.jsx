    ...
    <div className="max-w-md space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
        <div className="flex justify-between items-center mb-3">
          ...
          <Link to="/accounts" className="text-blue-500 text-xs">계좌 관리 →</Link>
        </div>
        {personalAccounts.length === 0 ? (
          <div className="text-center space-y-2 py-2">
            <p className="text-slate-400 text-sm">등록된 계좌가 없어요.</p>
            <Link to="/accounts" className="inline-block bg-slate-800 text-white text-sm px-4 py-2 rounded-lg hover:bg-slate-700 transition">
              계좌 만들기
            </Link>
          </div>
        ) : (
          <>
            <p className="text-xs text-slate-400 mb-1">총 잔액</p>
            <p className="text-2xl font-bold text-slate-900 mb-3">{totalBalance.toLocaleString()}원</p>
            <ul className="space-y-1.5">
              {personalAccounts.map(a => (
                <li key={a.id} className="flex justify-between items-center text-sm">
                  <Link to={`/accounts/${a.id}`} className="text-slate-700 hover:text-blue-500 transition">{a.name}</Link>
                  <span className="text-slate-500">{Number(a.balance).toLocaleString()}원</span>
                </li>
              ))}
            </ul>
          </>
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              {isKakao && <span className="text-xs bg-yellow-100 text-yellow-700 font-medium px-2 py-0.5 rounded-full">카카오 SNS 계정</span>}
            </div>
          </div>
          <div>
